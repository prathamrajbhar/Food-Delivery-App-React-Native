...
const SouthIndianFood = () => {
    const [SouthIndianFoodAPI, setSouthIndianFoodData] = useState([]);
    ...
    useEffect(() => {
        fetch('https://raw.githubusercontent.com/FoodieMunch/Foodie-Munch/main/FoodAPI.json')
            ...
            .then((data) => {
                if (data && data.SouthIndianFoodAPI) {
                    setSouthIndianFoodData(data.SouthIndianFoodAPI); ``
                }
            })
            .catch((error) => {
                console.error(error);
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.text}>South Indian Food</Text>
                <TouchableOpacity onPress={() => navigation.navigate('SouthIndianFoodViewAll')}>
                    <Text style={styles.ViewAll}>View All</Text>
                </TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.foodContainer}>
                {SouthIndianFoodAPI.map((item, index) => {
                    return (
                        <TouchableOpacity
                            key={index}
