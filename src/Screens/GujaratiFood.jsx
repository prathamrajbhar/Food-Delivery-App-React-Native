...
const GujaratiFood = () => {
    ...
    // Fetch data from the given JSON URL when the component mounts
    useEffect(() => {
        fetch(URL)
            .then((response) => response.json())
            .then((data) => {
                if (data && data.GujaratiFoodAPI) {
                    setGujaratiFoodData(data.GujaratiFoodAPI);
                }
            })
            .catch((error) => {
                console.error(error);
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.text}>Gujarati Food</Text>
                <TouchableOpacity onPress={() => navigation.navigate('GujaratiFoodViewAll')}>
