...
const PizzaFood = () => {
    const [PizzaFoodAPI, setPizzaFoodData] = useState([]);
        ...
        fetch('https://raw.githubusercontent.com/FoodieMunch/Foodie-Munch/main/FoodAPI.json')
            ...
            .then((data) => {
                if (data && data.PizzaFoodAPI) {
                    setPizzaFoodData(data.PizzaFoodAPI);
                }
            })
            .catch((error) => {
                console.error(error);
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.text}>Pizza Food</Text>
                <TouchableOpacity onPress={() => navigation.navigate('PizzaFoodViewAll')}>
                    <Text style={styles.ViewAll}>View All</Text>
                </TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.foodContainer}>
                {PizzaFoodAPI.map((item, index) => {
                    return (
                        <TouchableOpacity
                            key={index}
