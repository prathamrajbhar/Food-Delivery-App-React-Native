...
const RollsFood = () => {
    const [RollsFoodAPI, setRoolsFoodData] = useState([]);
    ...
    useEffect(() => {
        fetch('https://raw.githubusercontent.com/FoodieMunch/Foodie-Munch/main/FoodAPI.json')
            ...
            .then((data) => {
                if (data && data.RollsFoodAPI) {
                    setRoolsFoodData(data.RollsFoodAPI);
                }
            })
            .catch((error) => {
                console.error(error);
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.text}>Rolls Food</Text>
                <TouchableOpacity onPress={() => navigation.navigate('RollsFoodViewAll')}>
                    <Text style={styles.ViewAll}>View All</Text>
                </TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.foodContainer}>
                {RollsFoodAPI.map((item, index) => {
                    return (
                        <TouchableOpacity
                            key={index}
